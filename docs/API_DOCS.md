...
- Runtime docs are enabled by default outside production; set `API_DOCS_ENABLED=true` (or `false`) to override explicitly and optionally `API_DOCS_ADMIN_ONLY=false` to loosen access.
