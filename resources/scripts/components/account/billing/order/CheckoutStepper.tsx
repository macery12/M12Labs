        ...
        <nav aria-label="Progress" className="mb-8">
            <ol className="flex items-center justify-between">
                {steps.map((step, stepIdx) => (
                    <li key={step.name} className={classNames('relative', stepIdx !== steps.length - 1 ? 'pr-8 sm:pr-20 flex-1' : '')}>
                        {step.status === 'complete' ? (
                            <>
                                ...
                                <span className="absolute top-10 left-1/2 -translate-x-1/2 text-xs text-gray-400 whitespace-nowrap">
                                    {step.name}
                                </span>
                            </>
                        ) : step.status === 'current' ? (
                            ...
                            <>
                                ...
                                <span className="absolute top-10 left-1/2 -translate-x-1/2 text-xs text-gray-500 whitespace-nowrap">
                                    {step.name}
                                </span>
                            </>
                        )}
