...
class UpdateExtensionCommand extends Command
{
     ...
     */
    protected function resolveUpdateResolution(string $source): array
    {
        ...
        if ($source !== '') {
            ...
            if (count($matchingLocal) === 1) {
                if ($this->option('yes') || $this->confirm(sprintf(
                    'Found local package %s (%s) in %s. Update from that file instead of using the repository?',
                ...
                $choice = $this->choice(
                    ...
                    ['Use repository update', 'Select a discovered package', 'Enter a path', 'Cancel'],
                ...
                if ($choice === 'Cancel') {
                    throw new DisplayException('Update cancelled.');
                }
            }
        }
        ...
        if ($source === '') {
            ...
            if (count($validArtifacts) === 1) {
                if ($this->option('yes') || $this->confirm(sprintf(
                    'Update from the local package %s (%s) found in %s?',
    ...
    private function resolveFileModeSelection(string $source, string $cwd, array $discoveredArtifacts, array $validArtifacts): array
    {
        ...
        if ($validArtifacts === []) {
            throw new DisplayException('No local .M12LabsExtension files were found in the current directory. Pass a path or omit --file to update from a repository.');
        }
    ...
    private function selectDiscoveredArtifact(array $validArtifacts, string $cwd, array $discoveredArtifacts, ?string $requestedSource): array
    {
        ...
        $choice = $this->choice(
            sprintf('Found %d local extension package file(s) in %s. Select one to update from.', count($validArtifacts), $cwd),
            $choices,
            $choices[0]
        );
