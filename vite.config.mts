...
export default defineConfig(({ mode }) => {
    ...
    // babel-plugin-styled-components: disable displayName/fileName in production
    // (debug metadata only; these are already auto-detected from NODE_ENV but we
    // make the intent explicit so CI builds with unusual NODE_ENV still stay fast).
    const styledPlugin = [
        'babel-plugin-styled-components',
        { displayName: !isProduction, fileName: !isProduction, pure: true },
    ] as const;

    const plugins = [
        react({
            babel: {
                plugins: ['babel-plugin-macros', styledPlugin],
            },
        }),
    ];
